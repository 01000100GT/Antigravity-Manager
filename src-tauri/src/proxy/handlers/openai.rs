...
pub async fn handle_chat_completions(
    ...
    for attempt in 0..max_attempts {
        ...
        let (access_token, project_id, _email) = match token_manager.get_token(&model_group, None).await {
            Ok(t) => t,
            Err(e) => {
...
pub async fn handle_completions(
    ...
    for attempt in 0..max_attempts {
        ...
        let query_string = if openai_req.stream { Some("alt=sse") } else { None };

        // 3. Call Upstream
        match upstream.call_v1_internal(method, &access_token, gemini_body, query_string).await {
            Ok(response) => {
                let status = response.status();
                if !status.is_success() {
                     ...
                     // Retry logic
                     if status.as_u16() == 429 || status.as_u16() == 404 || status.as_u16() == 401 {
                        continue;
                     }
                ...
                if openai_req.stream {
                    ...
                    let body = if is_codex_style {
                        ...
                        let codex_stream = create_codex_sse_stream(Box::pin(gemini_stream), openai_req.model.clone());
                        Body::from_stream(codex_stream)
                    } else {
                        let legacy_stream = create_legacy_sse_stream(Box::pin(gemini_stream), openai_req.model.clone());
                        Body::from_stream(legacy_stream)
                    };
                    ...
                    return Ok(Response::builder()
                     ...
                     // 5. Handle Non-Streaming (Legacy Format)
                     let gemini_resp: Value = response.json().await
                        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
                     
                     let chat_resp = transform_openai_response(&gemini_resp);
                     
                     // Convert Chat Response -> Text Completion Response (or Codex Response)
                     // Note: Non-streaming for Codex CLI is rare but we handle it generally
                     ...
                     return Ok(Response::builder()
                         ...
                         .body(Body::from(serde_json::to_string(&legacy_resp).unwrap()))
                         .unwrap());
                }
            }
            Err(e) => {
                last_error = e.to_string();
                tracing::warn!("Retrying upstream request: {}", e);
                continue;
            }
        }
    }
    
    Err((StatusCode::TOO_MANY_REQUESTS, format!("All retries failed: {}", last_error)))
}
